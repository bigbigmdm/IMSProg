    ...
    , ui(new Ui::DialogNANDSr)
{
    ...
    int pixelsWidth = fm.horizontalAdvance("00> 4F 4E 46 49 00 00 00 00 02 00 00 00 00 00 00 00 ")
                      + 20;
...
void DialogNANDSr::on_pushButton_read_clicked()
{
    ...
    if (stCH341 == 0) {
        for (currRegister = 0; currRegister < 5; currRegister++) {
            if (RegNumbers[currRegister] != 0xff) {
                ...
                for (currBit = 0; currBit < 8; currBit++) {
                    currRegName = "lineEdit_sr" + QString::number(currRegister)
                                  + QString::number(currBit);
                    for (QLineEdit* edit : findChildren<QLineEdit*>()) {
        ...
        if ((buf[0] == 0x4f) && (buf[1] == 0x4e) && (buf[2] == 0x46) && (buf[3] == 0x49)) {
            ...
            buftxt.clear();
            for (i = 32; i < 44; i++)
                buftxt = buftxt + static_cast<char>(buf[i]);
            ...
            ui->textEdit_buf->setText(tr("The Parameter Page is not supported."));
        //Get unique ID
        SPI_CONTROLLER_Chip_Select_Low(programmerType);
        SPI_CONTROLLER_Write_One_Byte(0x13, programmerType);
        SPI_CONTROLLER_Write_One_Byte(0x00, programmerType);
            ...
            if (RegNumbers[currRegister] != 0xff) {
                for (currBit = 0; currBit < 8; currBit++) {
                    ...
                    for (QLineEdit* edit : findChildren<QLineEdit*>()) {
                        if (edit->objectName() == currRegName) {
                            currValue = edit->text();
                            if (QString::compare(currValue, "0", Qt::CaseInsensitive))
                                regData = regData + currByte;
                        }
                    }
                    currByte = static_cast<uint8_t>(currByte << 1);
                }
