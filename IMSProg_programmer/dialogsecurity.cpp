...
void DialogSecurity::setAlgorithm(uint8_t currentAlg)
{
    ...
    algSettings algSet[] = {
        ...
        {0x03,
         ...
         0,
         0,
         1}, //Winbond, Boya, UCUNDATA, Zetta, Zbit, XMC, Spansion
        {0x04,
         0,
         3,
         16,
         ...
         0,
         0,
         1}, //Gigadevice, Boya, UCUNDATA, Zetta, Puya, Zbit, XMC, XTX
    ...
    if (curCommands.ERSCUR == 0)
        ui->toolButton_erase->setDisabled(true);
    else
        ui->toolButton_erase->setDisabled(false);
...
void DialogSecurity::on_toolButton_read_clicked()
{
    ...
    if (curSettings.id > 0) {
        ...
        if (stCH341 == 0) {
            if (curRegister == 0)
                curRgAddr = curSettings.rg0addr;
            if (curRegister == 1)
                curRgAddr = curSettings.rg1addr;
            if (curRegister == 2)
                curRgAddr = curSettings.rg2addr;
            if (curRegister == 3)
                curRgAddr = curSettings.rg3addr;
            ...
            if (curSettings.a4byte == 1)
                SPI_CONTROLLER_Write_One_Byte(0x00, programmerType); //A32...A24
            ...
            if (curCommands.DUMRD == 1)
                SPI_CONTROLLER_Write_One_Byte(0x00, programmerType); //Dummy byte
...
void DialogSecurity::on_toolButton_write_clicked()
{
    ...
    if (curSettings.id > 0) {
        ...
        if (stCH341 == 0) {
            ...
            if (curSettings.size * 64 > 256)
                j = curSettings.size / 4;
            else
                j = 1;
            ...
            for (k = 0; k < j; k++) {
                ...
                SPI_CONTROLLER_Chip_Select_Low(programmerType);
                SPI_CONTROLLER_Write_One_Byte(curCommands.WRSCUR,
                                              programmerType); // Write SR command
                if (curSettings.a4byte == 1)
                    SPI_CONTROLLER_Write_One_Byte(0x00, programmerType); //A32...A24
                SPI_CONTROLLER_Write_One_Byte(a23a16, programmerType);   //A23...A16
                SPI_CONTROLLER_Write_One_Byte(a15a08, programmerType);   //A15...A08
                SPI_CONTROLLER_Write_One_Byte(0x00, programmerType);     //A07...A00
                if (curSettings.size < 4)
                    retval = SPI_CONTROLLER_Write_NByte(buf.get(),
                                                        curSettings.size * 64,
                                                        SPI_CONTROLLER_SPEED_SINGLE,
                                                        programmerType);
                else
                ...
                for (i = 0; i < 256; i++) {
                    ...
                    SPI_CONTROLLER_Chip_Select_Low(
                        programmerType); // waiting for WIP bit is set to zero
...
void DialogSecurity::on_toolButton_open_clicked()
{
    ...
    if (info.size() > curSettings.size * 64) {
        QMessageBox::about(this,
                           tr("Error"),
                           tr("The file size exceeds the security register size."));
        return;
    }
