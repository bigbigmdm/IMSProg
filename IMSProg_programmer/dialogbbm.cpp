...
void DialogBBM::on_pushButton_clicked()
{
    ...
    if (stCH341 == 0) {
        for (i = 0; i < static_cast<int>(totBlocks); i++) {
            retval = nand_checkBadBlock(static_cast<uint32_t>(i),
                                        static_cast<uint32_t>(sectSize),
            ...
            if (retval == 1) //(buf[0] != 0xff)
            {
                ...
                ui->tableWidgetScan->setItem(bbmCount,
                                             0,
                                             new QTableWidgetItem(QString::number(bbmCount + 1)));
        ...
        if (scanResult)
            ui->label_scan->setText(tr("Corrupted blocks found:"));
        else
            ui->label_scan->setText(tr("All blocks in the chip are good!"));
...
{
    setParams = settings;
    if ((setParams & 0x0f) == 0x01)
        ui->radioButton_w2->setChecked(true);
    if ((setParams & 0x0f) == 0x00)
        ui->radioButton_w1->setChecked(true);
    if ((setParams & 0xf0) == 0x10)
        ui->radioButton_e2->setChecked(true);
    if ((setParams & 0xf0) == 0x00)
        ui->radioButton_e1->setChecked(true);
}
...
void DialogBBM::on_pushButton_2_clicked()
{
    ...
    if (stCH341 == 0) {
        ...
        retval = SPI_CONTROLLER_Read_NByte(buf.get(),
                                           1,
        ...
        if ((buf[0] != 0xff) && (buf[1] != 0xff)) {
            ...
            for (i = 0; i < maxTableRows; i++) {
                ...
                if (stringResult == 0x00)
                    ui->tableWidgetBBM->setItem(i, 3, new QTableWidgetItem(tr("Not used")));
                if (stringResult == 0x02)
                    ui->tableWidgetBBM->setItem(i, 3, new QTableWidgetItem(tr("Active, valid")));
                if (stringResult == 0x03)
                    ui->tableWidgetBBM->setItem(i, 3, new QTableWidgetItem(tr("Active, not valid")));
            }
            ui->tableWidgetBBM->removeRow(maxTableRows);
            ui->tableWidgetBBM->resizeColumnsToContents();
        } else
            ui->label_bbm_result->setText(tr("BBM table is not used in this chip."));
        ProgDeviceClose(programmerType);
    } else
        QMessageBox::about(this,
                           tr("Error"),
...
{
    setParams = 0;
    if (ui->radioButton_e1->isChecked())
        setParams = setParams & 0x0f;
    if (ui->radioButton_e2->isChecked())
        setParams = setParams | 0x10;
    if (ui->radioButton_w1->isChecked())
        setParams = setParams & 0xf0;
    if (ui->radioButton_w2->isChecked())
        setParams = setParams | 0x01;
