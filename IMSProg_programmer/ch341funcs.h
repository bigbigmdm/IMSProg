...
#define mCH341_ENDP_INTER_UP \
    0x81 /* bEndpointAddress 0x81  EP 1 IN (Interrupt), unused on the source*/
#define mCH341_ENDP_INTER_DOWN \
    0x01 /* This endpoint isn't list on my lsusb -v output, unused on the source*/
...
int32_t ch341readEEPROM(struct libusb_device_handle *devHandle,
                        uint8_t *buf,
                        uint32_t bytes,
                        struct EEPROM *eeprom_info);
int32_t ch341writeEEPROM(struct libusb_device_handle *devHandle,
                         uint8_t *buf,
                         uint32_t bytes,
                         struct EEPROM *eeprom_info);
