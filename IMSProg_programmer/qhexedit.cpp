...
void QHexEdit::setCursorPosition(qint64 position)
{
    ...
    if (_readOnly)
        _cursorRect = QRect(_pxCursorX - pxOfsX,
...
qint64 QHexEdit::cursorPosition(QPoint pos)
{
    ...
    if ((posX >= _pxPosHexX) && (posX < (_pxPosHexX + (1 + _hexCharsInLine) * _pxCharWidth))) {
        ...
        result = _bPosFirst * 2 + x + y;
               ...
               && (posX < (_pxPosAsciiX + (1 + _bytesPerLine) * _pxCharWidth))) {
...
void QHexEdit::keyPressEvent(QKeyEvent *event)
{
    ...
    if (event->matches(QKeySequence::SelectNextPage)) {
        qint64 pos = _cursorPosition
                     + (((viewport()->height() / _pxCharHeight) - 1) * 2 * _bytesPerLine);
        setCursorPosition(pos);
        setSelection(pos);
    }
    ...
    if (!_readOnly) {
        ...
        if (event->matches(QKeySequence::Cut)) {
            QByteArray ba = _chunks
                                ->data(getSelectionBegin(), getSelectionEnd() - getSelectionBegin())
                                .toHex();
            ...
            if (event->matches(QKeySequence::Paste)) {
                ...
                if (event->matches(QKeySequence::Delete)) {
                    if (getSelectionBegin() != getSelectionEnd()) {
                        ...
                        if (_overwriteMode) {
                            QByteArray ba = QByteArray(getSelectionEnd() - getSelectionBegin(),
                                                       char(0));
                            replace(_bPosCurrent, ba.size(), ba);
                        } else {
                            ...
                            if (event->matches(QKeySequence::Redo)) {
                                ...
                                if ((((key >= '0' && key <= '9') || (key >= 'a' && key <= 'f'))
                                    ...
                                    if (getSelectionBegin() != getSelectionEnd()) {
                                        if (_overwriteMode) {
                                            qint64 len = getSelectionEnd() - getSelectionBegin();
                                            replace(getSelectionBegin(),
                                                    (int) len,
                                                    QByteArray((int) len, char(0)));
                                        } else {
                                            remove(getSelectionBegin(),
                                                   getSelectionEnd() - getSelectionBegin());
                                            _bPosCurrent = getSelectionBegin();
                                        }
                                        setCursorPosition(2 * _bPosCurrent);
                                        resetSelection(2 * _bPosCurrent);
                                    ...
                                    if (_chunks->size() > 0) {
                                        char ch = key;
                                        if (!_editAreaIsAscii) {
                                            QByteArray hexValue = _chunks->data(_bPosCurrent, 1)
                                                                      .toHex();
                                            if ((_cursorPosition % 2) == 0)
                                                hexValue[0] = key;
                                            else
...
void QHexEdit::paintEvent(QPaintEvent *event)
{
    ...
    if (event->rect() != _cursorRect) {
        ...
        if (_addressArea)
            painter.fillRect(QRect(-pxOfsX,
                                   event->rect().top(),
                                   _pxCharWidth * (_addrDigits + 1),
                                   height()),
                             _colorManager->notMarked(Area::Address).areaStyle());
        if (_asciiArea) {
            painter.fillRect(QRect(pxPosAsciiX2 - _pxAreaMargin,
                                   event->rect().top(),
        ...
        for (int row = 0, pxPosY = pxPosStartY; row <= _rowsShown; row++, pxPosY += _pxCharHeight) {
            ...
            if (_addressArea && row * _bytesPerLine < _dataShown.size()) {
                QString address = QString("%1").arg(_bPosFirst + row * _bytesPerLine
                                                        + _addressOffset,
                                                    _addrDigits,
                                                    16,
                                                    QChar('0'));
                ...
                painter.drawText(_pxPosAdrX - pxOfsX,
                                 pxPosY,
                                 hexCaps() ? address.toUpper() : address);
            }

            for (int colIdx = 0;
                 ((bPosLine + colIdx) < _dataShown.size() && (colIdx < _bytesPerLine));
                 colIdx++) {
                ...
                if (colIdx == 0)
                    ...
                    rect = QRect(pxPosX - _pxCharWidth,
                                 pxPosY - _pxCharHeight + _pxSelectionSub,
                                 3 * _pxCharWidth,
                                 _pxCharHeight);
...
{
    _dataShown = _chunks->data(_bPosFirst,
                               _bPosLast - _bPosFirst + _bytesPerLine + 1,
                               &_markedShown);
...
QString QHexEdit::toReadable(const QByteArray &ba)
{
    ...
    for (int i = 0; i < ba.size(); i += 16) {
        ...
        result += addrStr + " " + QString("%1").arg(hexStr, -48) + "  "
