    ...
    , ui(new Ui::MainWindow)
{
    ...
    grnKeyStyle = "QPushButton{color:#fff;background-color: rgb(120, 183, 140);border-radius: "
                  "20px;border: 2px solid "
                  "#094065;border-radius:8px;font-weight:600;}QPushButton::pressed{background-"
                  "color: rgb(115, 210, 22);}";
    redKeyStyle = "QPushButton{color:#fff;background-color:#f66;border-radius: 20px;border: 2px "
                  "solid #094065;border-radius:8px;font-weight:600;}";
    timer = new QTimer();
    ...
    //Reading ini file
    QString iniPath = QDir(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation))
                          .filePath("config.ini");
    ...
    if (QFileInfo(iniPath).exists()) {
        ...
        if (current_programmer == 0)
            ui->actionCH341A_B_v1_2->setChecked(true);
        if (current_programmer == 1)
            ui->actionCH341A_v1_7->setChecked(true);
    ...
    if (commandLineParams.count() > 1) {
        ...
        if ((commandLine.exists())
            && !(QString::compare(commandLine.suffix(), "bin", Qt::CaseInsensitive))) {
    ...
    if (statusCH341 == 0) {
        ...
        if (((currentNumBlocks > 0) && (currentBlockSize > 0) && (currentChipType == 0))
            ...
            for (k = 0; k < numBlocks; k++) {
                ...
                case 2:
                    //MicroWire
                    res = Read_EEPROM_3wire_param(buf.get(),
                                                  static_cast<int>(curBlock * step),
                                                  static_cast<int>(step),
                                                  static_cast<int>(currentChipSize),
                                                  currentAlgorithm);
                    if (res == 0)
                        res = 1;
                    break;
                ...
                case 6:
                    //NAND
                    if (nandRaw == true)
                        res = nand_block_read(buf.get(),
                                              currentPageSize + currentECCsize,
                                              k,
                                              sectorsPerBlock,
                                              current_programmer);
                    else
                ...
                if (res <= 0) {
                    QMessageBox::about(this,
                                       tr("Error"),
                                       tr("Error reading block ") + QString::number(curBlock));
                    ProgDeviceClose(current_programmer);
                ...
                addr = addr + step;
                if (curBlock * step < 0x500)
                    hexEdit->setData(chipData); //show buffer in hehedit while chip data is visible
                curBlock++;
                qApp->processEvents();
        ...
        QMessageBox::about(this,
                           tr("Error"),
                           tr("Programmer ") + ui->lStatus->text() + tr(" is not connected!"));
    }
    ProgDeviceClose(current_programmer);
    doNotDisturbCancel();
    filled = 0;
...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    if (((bufid[0] == 0xff) || (bufid[0] == 0x00)) && (bufid[1] != 0xff))
        ui->comboBox_type->setCurrentIndex(6);

    if ((currentChipType == 6) && ((bufid[0] == 0xff) || (bufid[0] == 0x00))) {
        nand_read_devid(bufid, 5, current_programmer);
    }

    ui->jedecEdit->setText(bytePrint(bufid[0]) + " " + bytePrint(bufid[1]) + " "
                           + bytePrint(bufid[2]));
    for (i = 0; i < max_rec; i++) {
        if ((bufid[0] == chips[i].chipJedecIDMan) && (bufid[1] == chips[i].chipJedecIDDev)
            && (bufid[2] == chips[i].chipJedecIDCap)) {
    ...
    if ((currentChipType == 5) && (bufid[0] == 0xff) && (bufid[1] == 0xff) && (bufid[2] == 0xff)) {
        ...
        if (!at45_read_sr(&sr)) {
            if ((sr & 0xC0) == 0x80) //45xx chip found
            {
                ...
                for (i = 0; i < max_rec; i++) {
                    if ((chips[i].chipTypeHex == 0x05)
                        && (chips[i].chipSize == ui->comboBox_size->currentData().toUInt())) {
...
void MainWindow::on_comboBox_size_currentIndexChanged(int index)
{
    ...
    if ((currentChipSize != 0) && (currentPageSize != 0) && (currentChipType > 0)) {
        ...
        preparingToCompare(1);
        numberOfReads = 0;
        if ((currentChipType == 6) && (nandRaw == true))
            chipData.resize(static_cast<int>(currentChipSize
                                             + currentChipSize / currentPageSize * currentECCsize));
        else
            chipData.resize(static_cast<int>(currentChipSize));
        chipData.fill(char(0xff));
        filled = 1;
...
void MainWindow::on_actionSave_triggered()
{
    ...
    if (QString::compare(info.suffix(), "bin", Qt::CaseInsensitive))
...
void MainWindow::on_actionErase_triggered()
{
    ...
    statusCH341
        = ProgDeviceInit(current_programmer,
                         currentChipType,
                         currentI2CBusSpeed); //ch341a_init(currentChipType, currentI2CBusSpeed);
    ch341StatusFlashing();
    if (statusCH341 != 0) {
        QMessageBox::about(this,
                           tr("Error"),
    ...
    if (currentChipType == 0) {
        ...
        if (currentNumBlocks > 0) {
            ...
            for (uint32_t curBlock = 0; curBlock < currentNumBlocks; curBlock++) {
                //config_stream(2);
                ret = snor_block_erase(curBlock,
                                       currentBlockSize,
                                       static_cast<u8>(currentAddr4bit),
                                       current_programmer);
                if (ret != 0) {
                    QMessageBox::about(this,
                                       tr("Error"),
                                       tr("Error erasing sector ") + QString::number(curBlock));
                    ProgDeviceClose(current_programmer);
                    doNotDisturbCancel();
                    return;
                }
    ...
    if (currentChipType == 1) {
        ...
        for (curBlock = 0; curBlock < numBlocks; curBlock++) {
            res = ch34xi2cBlockWrite(buf.get(),
                                     curBlock * step,
                                     step,
                                     currentPageSize,
                                     currentAlgorithm,
        ...
        if (numBlocks > 0) {
            ...
            for (uint32_t curBlock = 0; curBlock < numBlocks; curBlock++) {
                if ((nandSettings & 0xf0) == 0x00) {
                    retval = nand_checkBadBlock(static_cast<uint32_t>(curBlock),
                                                static_cast<uint32_t>(currentPageSize),
                                                static_cast<uint32_t>(sectInBlock),
                                                current_programmer);
                } else
                    retval = 0;
...
void MainWindow::on_actionOpen_triggered()
{
    ...
    if ((currentChipType == 6) && (nandRaw == true))
        arraySize = currentChipSize + currentChipSize / currentPageSize * currentECCsize;
    else
        arraySize = currentChipSize;

    if ((info.size() > arraySize) && (currentChipSize != 0)) {
        QMessageBox::about(this,
                           tr("Error"),
                           tr("The file size exceeds the chip size. Please select another chip or "
                              "file or use `Save part` to split the file."));
        return;
    }
...
void MainWindow::on_actionWrite_triggered()
{
    ...
    uint32_t numBlocks, step, sectorsPerBlock;
    statusCH341 = ProgDeviceInit(
        current_programmer,
        currentChipType,
        currentI2CBusSpeed); //statusCH341 = ProgDeviceInit(current_programmer);//ch341a_init(currentChipType, currentI2CBusSpeed);
    if (statusCH341 == 0) {
        ...
        if (((currentNumBlocks > 0) && (currentBlockSize > 0) && (currentChipType == 0))
            ...
            for (k = 0; k < numBlocks; k++) {
                ...
                case 2: //MicroWire
                    addrSrc = addrSrc + step;
                    res = Write_EEPROM_3wire_param(buf.get(),
                                                   static_cast<int>(curBlock * step),
                                                   static_cast<int>(step),
                                                   static_cast<int>(currentChipSize),
                                                   currentAlgorithm);
                ...
                case 6:
                    ...
                    if ((nandSettings & 0x0f) == 0x01) {
                        ...
                        badResult = nand_checkBadBlock(k,
                                                       currentPageSize,
                                                       sectorsPerBlock,
                                                       current_programmer);
                        if (badResult != 1)
                        ...
                        addrSrc = addrSrc + step;
                    if (nandRaw == true)
                        nand_block_write(buf.get(),
                                         currentPageSize + currentECCsize,
                                         k,
                                         sectorsPerBlock,
                                         current_programmer);
                    else
                        res = nand_block_write(buf.get(),
                                               currentPageSize,
                                               k,
                                               sectorsPerBlock,
                                               current_programmer);
                ...
                if (res <= 0) {
                    QMessageBox::about(this,
                                       tr("Error"),
                                       tr("Error writing sector ") + QString::number(curBlock));
                    doNotDisturbCancel();
                    ProgDeviceClose(current_programmer);
                    return;
                }
    ...
    if (max_rec > 0) {
        ...
        for (i = 0; i < max_rec; i++) {
            ...
            if (txt.compare(chips[i].chipManuf) == 0 && (currentChipType == chips[i].chipTypeHex)) {
                ...
                if (index2 == -1)
                    ui->comboBox_name->addItem(chips[i].chipName);
            }
        }
...
void MainWindow::on_comboBox_name_currentTextChanged(const QString& arg1)
{
    ...
    if (arg1.compare("") != 0) {
        for (i = 0; i < max_rec; i++) {
            if ((manName.compare(chips[i].chipManuf) == 0)
                && (arg1.compare(chips[i].chipName) == 0)) {
                index = ui->comboBox_size->findData(chips[i].chipSize);
                if (index != -1) { // -1 for not found
                    ui->comboBox_size->setCurrentIndex(index);
                }
        ...
        if ((currentChipSize != 0) && (currentPageSize != 0) && (currentChipType == 6)) {
            currentNumBlocks = currentChipSize / currentPageSize;
            if (nandRaw == true)
                chipData.resize(static_cast<int>(
                    currentChipSize + currentChipSize / currentPageSize * currentECCsize));
            else
                chipData.resize(static_cast<int>(currentChipSize));
            chipData.fill(char(0xff));
            filled = 1;
            ...
            for (k = 0; k < numBlocks; k++) {
                ...
                for (j = 0; j < step; j++) {
                    if (chipData[addr + j] != char(buf[addr + j - k * step])) {
                        ...
                        QMessageBox::about(this,
                                           tr("Error"),
                                           tr("Error comparing data!\nAddress:   ")
                                               + hexiAddr(addr + j) + tr("\nBuffer: ")
                                               + bytePrint(
                                                   static_cast<unsigned char>(chipData[addr + j]))
                                               + tr("    Chip: ")
                                               + bytePrint(buf[addr + j - k * step]));
                        ui->statusMessage->setText("");
                        ui->checkBox_3->setStyleSheet("");
                        ProgDeviceClose(current_programmer);
                        doNotDisturbCancel();
...
void MainWindow::on_pushButton_3_clicked()
{
    ...
    if (ui->checkBox->isChecked())
        MainWindow::on_actionErase_triggered();
    if (ui->checkBox_1->isChecked())
        MainWindow::on_actionCheck_erase_triggered();
    if (ui->checkBox_2->isChecked())
        MainWindow::on_actionWrite_triggered();
    if (ui->checkBox_3->isChecked())
        MainWindow::on_actionVerify_triggered();
...
void MainWindow::receiveAddr(QString addressData)
{
    ...
    if (endType.compare("*") == 0) {
        ...
        if (blockEndAddr < blockStartAddr) {
            QMessageBox::about(this,
                               tr("Error"),
                               tr("The end address must be greater than the starting address."));
            return;
        }
        if (static_cast<int>(blockEndAddr) > hexEdit->data().length()) {
            QMessageBox::about(this,
                               tr("Error"),
                               tr("The ending address must not exceed the buffer size."));
            return;
        }
        blockLen = blockEndAddr - blockStartAddr + 1;
    } else
        blockLen = hexToInt(addressData.mid(e + 1, t - e - 2));
...
void MainWindow::ch341StatusFlashing()
{
    if (statusCH341 == 0) {
        ...
        ui->eStatus->setStyleSheet(
            "QLineEdit {border: 2px solid gray;border-radius: "
            "5px;color:#000;background:#9f0;font-weight:600;border-style:inset;}");
    } else {
        ui->eStatus->setText(tr("Not connected"));
        ui->eStatus->setStyleSheet(
            "QLineEdit {border: 2px solid gray;border-radius: "
            "5px;color:#fff;background:#f00;font-weight:600;border-style:inset;}");
    }
}
...
void MainWindow::on_comboBox_type_currentIndexChanged(int index)
{
    ...
    for (i = 0; i < max_rec; i++) {
        ...
        if ((index2 == -1) && (chips[i].chipTypeHex == currentChipType))
    ...
    if (index != 0)
        ui->actionSecurity_registers->setEnabled(false);
...
{
    if ((currentChipType == 0) || (currentChipType == 5) || (currentChipType == 6))
        ui->actionDetect->setDisabled(false);
    ...
    ui->actionShow_programmer_version->setDisabled(true);
    if ((currentChipType == 0) || (currentChipType == 6) || (currentChipType > 2))
        ui->actionChip_info->setDisabled(false);
    if ((currentChipType == 0) || (currentChipType == 6))
        ui->actionSecurity_registers->setDisabled(false);
    if (currentChipType == 6)
        ui->actionBad_block_management->setDisabled(false);
    ui->actionStop->setDisabled(true);
...
void MainWindow::on_pushButton_4_clicked()
{
    ...
    if ((currentChipType == 0) && (ui->comboBox_vcc->currentIndex() == 1))
        infoDialog->setChip(2); //NOR_FLASH 1.8
    if ((currentChipType == 0) && (ui->comboBox_vcc->currentIndex() == 2))
        infoDialog->setChip(3); //NOR FLASH 3.3
    if ((currentChipType == 0) && (ui->comboBox_vcc->currentIndex() == 3))
        infoDialog->setChip(8); //NOR FLASH 2.5
    if ((currentChipType == 1) && (ui->comboBox_vcc->currentIndex() == 1))
        infoDialog->setChip(1); //24xxx 3.3
    if ((currentChipType == 2) && (ui->comboBox_vcc->currentIndex() == 1))
        infoDialog->setChip(4); //93xxx 3.3
    if ((currentChipType == 3) && (ui->comboBox_vcc->currentIndex() == 1))
        infoDialog->setChip(2); //25xxx 3.3
    if ((currentChipType == 4) && (ui->comboBox_vcc->currentIndex() == 1))
        infoDialog->setChip(2); //95xxx 3.3
    if ((currentChipType == 5) && (ui->comboBox_vcc->currentIndex() == 1))
        infoDialog->setChip(5); //45xxx 3.3
    if ((currentChipType == 6) && (ui->comboBox_vcc->currentIndex() == 1))
        infoDialog->setChip(6); //NAND_FLASH 3.3
    if ((currentChipType == 6) && (ui->comboBox_vcc->currentIndex() == 2))
        infoDialog->setChip(7); //NAND_FLASH 1.8
}
...
void MainWindow::progInit()
{
    ...
    while (dataPoz < dataSize) {
        ...
        if (tmpBuf == 0x00)
            chips[recNo].chipVCC = "3.3 V";
        if (tmpBuf == 0x01)
            chips[recNo].chipVCC = "1.8 V";
        if (tmpBuf == 0x02)
            chips[recNo].chipVCC = "5.0 V";
        if (tmpBuf == 0x03)
            chips[recNo].chipVCC = "2.5 V";
    ...
    for (i = 0; i < max_rec; i++) {
        //replacing items to combobox Manufacture
        index2 = ui->comboBox_man->findText(chips[i].chipManuf);
        if ((index2 == -1) && (chips[i].chipTypeHex == 0))
            ui->comboBox_man->addItem(chips[i].chipManuf);
    }
    ui->comboBox_man->setCurrentIndex(0);
    ui->statusMessage->setText("");
...
void MainWindow::on_actionSecurity_registers_triggered()
{
    if (currentChipSize == 0) {
        QMessageBox::about(
            this,
            tr("Error"),
            tr("Before working with the security registers, click the 'Detect' button"));
        return;
    }
    if (currentChipType == 0) {
        if (currentAlgorithm > 0) {
            ...
            QMessageBox::about(this,
                               tr("Error"),
                               tr("There are no security registers in this chip or the current "
                                  "version of IMSProg does not support this algorithm."));
    }
    if (currentChipType == 6) {
...
void MainWindow::on_actionExport_to_Intel_HEX_triggered()
{
    ...
    ui->statusMessage->setText("");
    lastDirectory = info.filePath();
    if (QString::compare(info.suffix(), "hex", Qt::CaseInsensitive))
    ...
    while ((addr + hi_addr * 0x10000) < (chipData.size())) {
        if (addr < currSize - 0x20)
            ostatok = 0x20;
        else
            ostatok = currSize - addr;

        result.append(":");
        result.append(bytePrint(static_cast<unsigned char>(ostatok))); //number of bytes per string
        result.append(
            bytePrint(static_cast<unsigned char>((addr & 0xff00) >> 8)));  //one address byte
        result.append(bytePrint(static_cast<unsigned char>(addr & 0xff))); //zero address byte
        result.append("00");                                               //type 00
        counter = counter + static_cast<unsigned char>(ostatok) + uint8_t((addr & 0xff00) >> 8)
                  + uint8_t(addr & 0xff);
        for (i = 0; i < ostatok; i++) {
            result.append(
                bytePrint(static_cast<unsigned char>((chipData[hi_addr * 256 * 256 + addr + i]))));
...
void MainWindow::on_actionImport_from_Intel_HEX_triggered()
{
    ...
    while (!file.atEnd()) {
        ...
        counter = counter + static_cast<unsigned char>((lo_addr) >> 8)
                  + static_cast<unsigned char>(lo_addr & 0x00ff);
        ...
        if (command == 0) //reading bytes from current string
        {
            for (i = 0; i < lineLen; i++) {
                ...
                if (hi_addr * 256 * 256 + lo_addr + i > static_cast<unsigned long>(chipSize)) {
                    QMessageBox::about(this,
                                       tr("Error"),
                                       tr("The address is larger than the size of the chip!"));
                    file.close();
                    fileName.clear();
                    hexEdit->setData(chipData);
                    ui->progressBar->setValue(0);
        ...
        if (command == 4) //Changing the high address
        {
            ...
            counter = counter + static_cast<unsigned char>((hi_addr) >> 8)
                      + static_cast<unsigned char>(hi_addr & 0x00ff);
            counter = 255 - counter + 1;
...
void MainWindow::on_actionFill_test_image_triggered()
{
    ...
    while (curPos < fileSize) {
        ...
        for (j = addrSize - 1; j >= 0; j--) {
            if (digits[j] < 10)
                digits[j] = digits[j] + 0x30;
            else
                digits[j] = digits[j] + 0x37;
        ...
        curPos = curPos + 16;
        if (curPos % 512 == 0)
            ui->progressBar->setValue(static_cast<int>(curPos >> divider));
    }
    hexEdit->setData(chipData);
    ui->crcEdit->setText(getCRC32(chipData));
    ui->progressBar->setValue(0);
...
void MainWindow::on_actionShow_programmer_version_triggered()
{
    ...
    if (statusCH341 == 0) {
        ...
        QMessageBox::about(this,
                           tr("Info:"),
                           (tr("Device revision: ") + byteBCDPrint(buf[13], false) + "."
                            + byteBCDPrint(buf[12], true)));
    } else
        QMessageBox::about(this,
                           tr("Error"),
                           tr("Programmer ") + ui->lStatus->text() + tr(" is not connected!"));
...
void MainWindow::on_comboBox_raw_currentIndexChanged(int index)
{
    if (index == 0) {
        nandRaw = false;
        if (scroll_connect)
            disconnect(scroll_connect);
    } else {
        nandRaw = true;
        scroll_connect = connect(hexEdit->verticalScrollBar(),
                                 &QScrollBar::valueChanged,
                                 this,
                                 &MainWindow::handleScroll);
    }
...
void MainWindow::handleScroll()
{
    ...
    if (posStart > 0x40) {
        for (i = posStart; i <= posEnd; i = i + 16) {
            if (i % (currentPageSize + currentECCsize) == 0)
                hexEdit->addUserArea(i - currentECCsize,
                                     i,
                                     QColor(0, 0, 0, 255),
                                     QColor(255, 0, 0, 60));
        }
