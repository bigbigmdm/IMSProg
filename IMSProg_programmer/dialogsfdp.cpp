...
void DialogSFDP::legendPrint(QString basic, QString extended, QString manufacture)
{
    ...
    ui->label_9->setText(h1 + basic + h0 + l1 + h2 + extended + h0 + l2 + h3 + manufacture + h0 + l3
                         + h4);
}
...
void DialogSFDP::on_pushButton_clicked()
{
    ...
    if (stCH341 == 0) {
        ...
        retval = SPI_CONTROLLER_Read_NByte(sfdpBuf.get(),
                                           3,
        ...
        SPI_CONTROLLER_Chip_Select_Low(programmerType);
        SPI_CONTROLLER_Write_One_Byte(0x5a, programmerType);
        ...
        if ((sfdpBuf[0] == 0x53) && (sfdpBuf[1] == 0x46) && (sfdpBuf[2] == 0x44)
            && (sfdpBuf[3] == 0x50)) {
        ...
        if (sfdpSupport) {
            if ((sfdpBuf[0x80] == 0x53) && (sfdpBuf[0x81] == 0x46) && (sfdpBuf[0x82] == 0x44)
                && (sfdpBuf[0x83] == 0x50))
                imax = 127;
            else
                imax = 254;
            ...
            if (manufAreaAddress != 0xff) {
                if ((sfdpBuf[manufAreaAddress] != 0xff) && (sfdpBuf[manufAreaAddress + 1] != 0xff)) {
                    VCCmax = bytePrint(sfdpBuf[manufAreaAddress + 1])
                             + bytePrint(sfdpBuf[manufAreaAddress]);
                    VCCmin = bytePrint(sfdpBuf[manufAreaAddress + 3])
                             + bytePrint(sfdpBuf[manufAreaAddress + 2]);
                    VCCmax.insert(1, ".");
                    VCCmin.insert(1, ".");
                ...
                if (sfdpBuf[manufAreaAddress + 9] != 0xff) {
                    if ((sfdpBuf[manufAreaAddress + 9] & 0x08) != 0)
                        ui->lineEdit_otp->setText("Yes");
                    if ((sfdpBuf[manufAreaAddress + 9] & 0x08) == 0)
                        ui->lineEdit_otp->setText("No");
                } else
                    ui->lineEdit_otp->setText("");
            } else {
                ui->lineEdit_vcc_max->setText("");
            ...
            sfdpSize = (sfdpBuf[twoAreaAddress + 4] + sfdpBuf[twoAreaAddress + 5] * 256
                        + sfdpBuf[twoAreaAddress + 6] * 256 * 256
                        + sfdpBuf[twoAreaAddress + 7] * 256 * 256 * 256 + 1)
                       / 8 / 1024;
            ...
            if (sfdpBuf[twoAreaAddress + 0x0f] == 0xbb)
                speeds = speeds + "/Dual";
            if (sfdpBuf[twoAreaAddress + 0x09] == 0xeb)
                speeds = speeds + "/Quad";
            ...
            for (i = 0; i <= imax; i++) {
                if (i % 16 == 0)
                    regData = regData + "<br> ";
                if (i == 0x0c)
                    regData = regData + "<span style=\" background:#f77;\">";
                if (i == 0x0d)
                    regData = regData + "</span>";

                if ((i == 0x14) && (jedecMan == sfdpBuf[0x10]) && (sfdpBuf[0x06] != 0))
                    regData = regData + "<span style=\" background:#7f7;\">";
                if ((i == 0x15) && (jedecMan == sfdpBuf[0x10]) && (sfdpBuf[0x06] != 0))
                    regData = regData + "</span>";
                if ((i == 0x1c) && (jedecMan != sfdpBuf[0x10]) && (sfdpBuf[0x06] != 0))
                    regData = regData + "<span style=\" background:#7f7;\">";
                if ((i == 0x1d) && (jedecMan != sfdpBuf[0x10]) && (sfdpBuf[0x06] != 0))
                    regData = regData + "</span>";

                if (i == twoAreaAddress)
                    regData = regData + "<span style=\" background:#f77;\">";
                if (i == twoAreaAddress + twoAreaLen)
                    regData = regData + "</span>";
                if (i == manufAreaAddress)
                    regData = regData + "<span style=\" background:#7f7;\">";
                if (i == manufAreaAddress + manAreaLen)
                    regData = regData + "</span>";
        ...
        numOfRegisters = 0;
        SPI_CONTROLLER_Chip_Select_Low(programmerType);
        SPI_CONTROLLER_Write_One_Byte(0x35, programmerType);
        retval = SPI_CONTROLLER_Read_NByte(sfdpBuf.get(),
                                           2,
                                           SPI_CONTROLLER_SPEED_SINGLE,
                                           programmerType);
        ...
        if (sfdpBuf[0] != 0xff) {
            ...
            setRegStatus(1, false);

        //READING STATUS REGISTER 2
        SPI_CONTROLLER_Chip_Select_Low(programmerType);
        SPI_CONTROLLER_Write_One_Byte(0x15, programmerType);
        retval = SPI_CONTROLLER_Read_NByte(sfdpBuf.get(),
                                           2,
                                           SPI_CONTROLLER_SPEED_SINGLE,
                                           programmerType);
        ...
        if (sfdpBuf[0] != 0xff) {
            ...
            setRegStatus(2, false);

        //Reading Unique ID
        SPI_CONTROLLER_Chip_Select_Low(programmerType);
        SPI_CONTROLLER_Write_One_Byte(0x4b, programmerType);
        SPI_CONTROLLER_Write_One_Byte(0x00, programmerType);
        SPI_CONTROLLER_Write_One_Byte(0x00, programmerType);
        SPI_CONTROLLER_Write_One_Byte(0x00, programmerType);
        SPI_CONTROLLER_Write_One_Byte(0x00, programmerType);
        ...
        if ((sfdpBuf[0x00] == sfdpBuf[0x08]) && (sfdpBuf[0x01] == sfdpBuf[0x09])
            && (sfdpBuf[0x02] == sfdpBuf[0x0a]) && (sfdpBuf[0x03] == sfdpBuf[0x0b]))
            idSize = 8;
        else
            idSize = 16;
        regData = "";
        ...
        QMessageBox::about(this,
                           tr("Error"),
                           tr("Programmer ") + programmerName + tr(" is not connected!"));
}
...
void DialogSFDP::on_pushButton_3_clicked()
{
    if (numOfRegisters < 3) {
        ...
        if (stCH341 == 0) {
                ...
                for (j = 7; j >= 0; j--) {
                    ...
                    for (QLineEdit* edit : findChildren<QLineEdit*>()) {
                        if (edit->objectName() == currRegName) {
                            if (QString::compare(edit->text(), "0", Qt::CaseInsensitive))
                                r[k] = r[k] + i;
                            i = i / 2;
                        }
                    }
            ...
            SPI_CONTROLLER_Chip_Select_Low(programmerType);
            SPI_CONTROLLER_Write_One_Byte(0x50, programmerType);
            SPI_CONTROLLER_Chip_Select_High(programmerType);
            usleep(1);

            SPI_CONTROLLER_Chip_Select_Low(programmerType);
            SPI_CONTROLLER_Write_One_Byte(0x01, programmerType);
            SPI_CONTROLLER_Write_One_Byte(r[0], programmerType);
            if (numOfRegisters > 0)
                SPI_CONTROLLER_Write_One_Byte(r[1], programmerType);
            SPI_CONTROLLER_Chip_Select_High(programmerType);
            usleep(1);
            ...
            if (numOfRegisters > 0) {
                ...
                SPI_CONTROLLER_Chip_Select_Low(programmerType);
                SPI_CONTROLLER_Write_One_Byte(0x31, programmerType);
                SPI_CONTROLLER_Write_One_Byte(r[1], programmerType);
                SPI_CONTROLLER_Chip_Select_High(programmerType);
                usleep(1);
            ...
            if (numOfRegisters > 1) {
                ...
                SPI_CONTROLLER_Chip_Select_Low(programmerType);
                SPI_CONTROLLER_Write_One_Byte(0x11, programmerType);
                SPI_CONTROLLER_Write_One_Byte(r[2], programmerType);
                SPI_CONTROLLER_Chip_Select_High(programmerType);
                usleep(1);
        ...
        QMessageBox::about(this,
                           tr("Error"),
                           tr("Before writing the registers, please press the `Read` button!"));
}
...
void DialogSFDP::setDeviceType(const uint8_t pType)
{
    programmerType = pType;
    if (programmerType < 2)
        programmerName = "CH341A";
    if (programmerType == 2)
        programmerName = "CH347T";
}
