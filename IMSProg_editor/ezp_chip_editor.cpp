...
void MainWindow::on_actionOpen_triggered()
{
    ...
    while (dataPoz < dataSize) {
        ...
        chips[recNo].chipJedecID = "0x" + bytePrint(manCode) + bytePrint(chipID)
                                   + bytePrint(chipSizeCode);
        ...
        if (chips[recNo].chipTypeTxt.compare("SPI_FLASH") == 0)
            model->item(recNo, 0)->setBackground(QBrush(QColor(0xcc, 0xff, 0xff)));
        else if (chips[recNo].chipTypeTxt.compare("24_EEPROM") == 0)
            model->item(recNo, 0)->setBackground(QBrush(QColor(0xff, 0xff, 0xcc)));
        else if (chips[recNo].chipTypeTxt.compare("93_EEPROM") == 0)
            model->item(recNo, 0)->setBackground(QBrush(QColor(0xff, 0xcc, 0xcc)));
        else if (chips[recNo].chipTypeTxt.compare("95_EEPROM") == 0)
            model->item(recNo, 0)->setBackground(QBrush(QColor(0xcc, 0xff, 0xcc)));
        else if (chips[recNo].chipTypeTxt.compare("25_EEPROM") == 0)
            model->item(recNo, 0)->setBackground(QBrush(QColor(0xff, 0xcc, 0xff)));
        else if (chips[recNo].chipTypeTxt.compare("45_EEPROM") == 0)
            model->item(recNo, 0)->setBackground(QBrush(QColor(0xcc, 0xcc, 0xff)));
        else if (chips[recNo].chipTypeTxt.compare("SPI_NAND") == 0)
            model->item(recNo, 0)->setBackground(QBrush(QColor(0xcc, 0xcc, 0xcc)));
...
QString MainWindow::sizeConvert(int a)
{
    ...
    rez = "0";
    if (a < 1024)
        rez = QString::number(a) + " B";
    else if ((a < 1024 * 1024) || (a % (1024 * 1024) > 0))
        rez = QString::number(a / 1024) + " K";
    else
        rez = QString::number(a / 1024 / 1024) + " M";
    return rez;
}
void MainWindow::onDataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight)
{
    if (ui->tableView->model() != nullptr) {
        ...
        if (row >= 0) {
            ...
            if (tmpStr.compare("SPI_FLASH") == 0)
                model->item(row, 0)->setBackground(QBrush(QColor(0xcc, 0xff, 0xff)));
            else if (tmpStr.compare("24_EEPROM") == 0)
                model->item(row, 0)->setBackground(QBrush(QColor(0xff, 0xff, 0xcc)));
            else if (tmpStr.compare("93_EEPROM") == 0)
                model->item(row, 0)->setBackground(QBrush(QColor(0xff, 0xcc, 0xcc)));
            else if (tmpStr.compare("95_EEPROM") == 0)
                model->item(row, 0)->setBackground(QBrush(QColor(0xcc, 0xff, 0xcc)));
            else if (tmpStr.compare("25_EEPROM") == 0)
                model->item(row, 0)->setBackground(QBrush(QColor(0xff, 0xcc, 0xff)));
            else if (tmpStr.compare("45_EEPROM") == 0)
                model->item(row, 0)->setBackground(QBrush(QColor(0xcc, 0xcc, 0xff)));
            else if (tmpStr.compare("SPI_NAND") == 0)
                model->item(row, 0)->setBackground(QBrush(QColor(0xcc, 0xcc, 0xcc)));
            else
                model->item(row, 0)->setBackground(QBrush(QColor(0xff, 0xff, 0xff)));
        }
    }
}
...
void MainWindow::on_actionSave_triggered()
{
    ...
    if (ui->tableView->model() != nullptr) {
        ...
        while (recNo < rowCount) {
            ...
            if (tmpStr[tmpStr.size() - 1] == 'B')
                delitel = 1;
            if (tmpStr[tmpStr.size() - 1] == 'K')
                delitel = 1024;
            if (tmpStr[tmpStr.size() - 1] == 'M')
                delitel = 1024 * 1024;
            ...
            if (tmpStr[4] == 'V') {
                if (tmpStr.compare("3.3 V") == 0)
                    toSave[recNo * 0x44 + 0x43] = 0x00;
                if (tmpStr.compare("5.0 V") == 0)
                    toSave[recNo * 0x44 + 0x43] = 0x02;
                if (tmpStr.compare("1.8 V") == 0)
                    toSave[recNo * 0x44 + 0x43] = 0x01;
                if (tmpStr.compare("2.5 V") == 0)
                    toSave[recNo * 0x44 + 0x43] = 0x03;
            }
            recNo++;
        }
        // 0x44 zero bytes
        ...
        for (int i = 0; i < selection.count(); i++) {
            ...
            if (index.row() > 0) {
                for (int j = 0; j < 13; j++) {
                    ...
                    tmpStr = ui->tableView->model()->data(indTo).toString();
                    ui->tableView->model()
                        ->setData(indTo, ui->tableView->model()->data(indFrom).toString());
                    ui->tableView->model()->setData(indFrom, tmpStr);
    ...
    if (ui->tableView->model() != nullptr) {
        ...
        for (int i = 0; i < selection.count(); i++) {
            ...
            for (j = 0; j < 13; j++) {
                ...
                if (j != 12)
                    toCSV = toCSV + ui->tableView->model()->data(indFrom).toString() + ";";
                else
                    toCSV = toCSV + ui->tableView->model()->data(indFrom).toString();
            }
            toCSV = toCSV + "\n";
    ...
    if (ui->tableView->model() != nullptr) {
        ...
        if (curPoz >= 0) //header detected
        {
            ...
            while (1) {
                ...
                for (j = 0; j < 13; j++) {
                    if (j == 12)
                        curPoz = fromCSV.indexOf("\n", fromPoz);
                    else
                        curPoz = fromCSV.indexOf(separator, fromPoz);
                    if (curPoz >= 0) {
                    ...
                    if (j == 0) {
                        ...
                        if (!correctChipTyte(tmpStr)) {
                            QMessageBox::warning(this,
                                                 tr("Warning"),
                                                 tr("Invalid CSV data file format."));
                            return;
                        }
                    }
                    //add string, ic_type->add cell
                    if (j == 12) {
                        if (chType.compare("SPI_FLASH") == 0)
                            model->item(recNo, 0)->setBackground(QBrush(QColor(0xcc, 0xff, 0xff)));
                        else if (chType.compare("24_EEPROM") == 0)
                            model->item(recNo, 0)->setBackground(QBrush(QColor(0xff, 0xff, 0xcc)));
                        else if (chType.compare("93_EEPROM") == 0)
                            model->item(recNo, 0)->setBackground(QBrush(QColor(0xff, 0xcc, 0xcc)));
                        else if (chType.compare("95_EEPROM") == 0)
                            model->item(recNo, 0)->setBackground(QBrush(QColor(0xcc, 0xff, 0xcc)));
                        else if (chType.compare("25_EEPROM") == 0)
                            model->item(recNo, 0)->setBackground(QBrush(QColor(0xff, 0xcc, 0xff)));
                        else if (chType.compare("45_EEPROM") == 0)
                            model->item(recNo, 0)->setBackground(QBrush(QColor(0xcc, 0xcc, 0xff)));
                        else if (chType.compare("SPI_NAND") == 0)
                            model->item(recNo, 0)->setBackground(QBrush(QColor(0xcc, 0xcc, 0xcc)));
                        else
                            model->item(recNo, 0)->setBackground(QBrush(QColor(0xff, 0xff, 0xff)));
                    }
